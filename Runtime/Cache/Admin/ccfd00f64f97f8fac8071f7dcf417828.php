...
<div style="margin:0 auto; width:98%;">
	...
	<form action="<?php echo U('Save');?>" method="post" name="form1">
		<?php if(is_array($UserInfo)): $i = 0; $__LIST__ = $UserInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
			...
			<input type='text' name='vipname' value='<?php echo ($vo["vipname"]); ?>'/>[<small style='color:red'>*</small>]
		...
		<tr>
			...
			<td><input type='text' name='level' value='<?php echo ($vo["viplevel"]); ?>'/>[<small style='color:red'>%</small>]</td>
		</tr>
		<tr>
			<td style="text-align:center;">等级积分：</td>
			<td>每达到<input type='text' name='yuan'  value='<?php echo ($vo["yuan"]); ?>' style="width:40px"/>[<small style='color:red'>元</small>]，
            	可得<input type='text' name='scores' value='<?php echo ($vo["sores"]); ?>' style=" width:40px"/>积分。
            </td>
		</tr>
