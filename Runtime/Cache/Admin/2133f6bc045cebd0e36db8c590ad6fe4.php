...
<link rel="stylesheet" type="text/css" href="__PUB__style/NewCss2.css"/>
<style type="text/css">
...
<script type="text/javascript">
		...
		window.onload = function() {
			var settings = {
				...
				upload_url: "__APP__/Admin/Upload/Uplaod/?id=<?php echo ($_GET['cid']); ?>",	
...
<div style="margin:0 auto; width:98%;">
	...
	<?php if(is_array($list_kc)): $i = 0; $__LIST__ = $list_kc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><form action="__APP__/Admin/Commodity/Save/id/<?php echo ($vo["id"]); ?>" method="post" name="form1" enctype="multipart/form-data">
		
		<tr>
			<td width="10%" style="text-align:center;">标题：</th>
			
			<td width="30%">
			<input type='hidden' name='id' value='<?php echo ($vo["Clasid"]); ?>'/>
			<input type='text' name='title' value='<?php echo ($vo["title"]); ?>'/>[<small style='color:red'>*</small>]
		...
		<tr>
			...
			<td><input type='text' name='model' value='<?php echo ($vo["model"]); ?>'/>[<small style='color:red'>*</small>]</td>
		</tr>
		<tr>
			<td style="text-align:center;">商城地址：</td>
			<td><input type='text' name='mall' value='<?php echo ($vo["mall"]); ?>'/>[<small style='color:red'>*</small>]</td>
		</tr>
		<tr>
	...
	<tr>
			...
			<td>
				<input type="text" name="Size" value='<?php echo ($vo["Size"]); ?>' id="Host" >
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="Color" value='<?php echo ($vo["Color"]); ?>' id="Host" >
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="CZ" value='<?php echo ($vo["CZ"]); ?>' id="Host" >
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<select name='Sj'>
					<?php if($vo["ShangJ"] == 0): ?><option value='0'>上架</option>
						<option value='1'>下架</option>
					<?php else: ?>
						<option value='1'>下架</option>
						<option value='0'>上架</option><?php endif; ?>
		...
		<tr>
			...
			<td>
				<input type="text" value='<?php echo ($vo["Number"]); ?>' name="Number" id="Host" >
		...
		<tr>
			...
			<td>
				<input type="text" value='<?php echo ($vo["Pre"]); ?>' name="Pre" id="Host" >
			</td>
		</tr>
		...
		<tr>
			...
			<td>
				<input type="text" value='<?php echo ($vo["Unit"]); ?>' name="unit" id="Host" >
			</td>
		</tr>
	...
	<tr>
		...
		<tr >
			...
			<td colspan="3">
				<button class="btn btn-large btn-info" type="button" onclick='Submit();'>提交</button>
